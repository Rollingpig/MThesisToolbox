...
def draw_validation_img():
    fig, ax = plt.subplots(nrows=1, ncols=2, constrained_layout=True, figsize=(6, 1.7))

    # 绘制左图的模拟数据
    simulation_file = ['SA_simulation/Test0-' + str(i) + '_flow.csv' for i in range(30)]
    simulation_flows = [read_simulation(simulation_file[i]).averaging() for i in range(30)]
    draw_line(simulation_flows, ax[0], color='wheat')
    ...
    draw_line([total_flow.averaging()], ax[0], color='tomato')
    ...
    draw_line([Flow([read_simulation(simulation_file[i]).cumulate() for i in range(30)], multi_input=True) / 30],
              ax[1], color='gold')
    draw_line([total_flow.cumulate().values()], ax[1], color='tomato')
    ...
    # plt.savefig('validation-low.png', dpi=120)
